...
if __name__ == '__main__':
    ...
    lowest_distance_matrix = []
    for r1, r2 in zip(distances1, distances2):
        low_row = [d1 if d1 < d2 else d2 for (d1, d2) in zip(r1, r2)]
        lowest_distance_matrix.append(low_row)
    
    max_dist = 0
    for r in lowest_distance_matrix:
        for v in r:
            if v > max_dist and v != np.inf:
                max_dist = v
